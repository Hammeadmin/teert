...
const handleViewDetails = useCallback(async (posting: JobPosting) => {
    ...
    const handleUpdateStatus = async (postingId: string, newStatus: 'open' | 'filled' | 'cancelled' | 'completed') => {
        if (newStatus === 'filled') {
            toast.info("För att markera en annons som 'Tillsatt' och koppla en anställd för lön, vänligen acceptera en sökande via 'Hantera Sökande' från din Dashboard eller när du ser annonsdetaljerna.", {duration: 8000});
