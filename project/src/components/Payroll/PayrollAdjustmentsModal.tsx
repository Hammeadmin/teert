...
interface PayrollAdjustmentsModalProps {
    ...
    onClose: () => void;
    currentRecordType: 'shift' | 'posting'; // ADDED: To receive the type from parent
}
...
export const PayrollAdjustmentsModal: React.FC<PayrollAdjustmentsModalProps> = ({
    ...
    const handleSave = async () => {
        setIsLoading(true);
        setError(null);
        console.log("[PayrollAdjustmentsModal] payrollRecord.id:", payrollRecord.id);
        console.log("[PayrollAdjustmentsModal] currentRecordType passed to updatePayrollAdjustments:", currentRecordType);

        // Use the passed currentRecordType instead of payrollRecord.record_type
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="flex justify-end items-center p-3 sm:p-4 border-t bg-gray-50 space-x-2">
                    <button onClick={onClose} type="button" className="btn btn-secondary btn-sm sm:btn-md">Avbryt</button>
